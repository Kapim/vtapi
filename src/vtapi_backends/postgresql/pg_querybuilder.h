...
public:
     ...
     * @param isull specify IS NOT NULL (false) / IS NULL (true) behavior
    ...
    explicit PGQueryBuilder(PGConnection &connection)
        : QueryBuilder(connection), _cnt_param(0) {}
...
private:
    ...
    uint _cnt_param;            /**< keys counter */


    std::string constructTable(const std::string& table = std::string(),
                       const std::string& schema = std::string()) const;
    std::string constructColumn(const std::string& column,
                        const std::string& table = std::string()) const;
    std::string constructColumnNoTable(const std::string& column) const;
    ...
    PGtimestamp UnixTimeToTimestamp(const std::chrono::system_clock::time_point & utime) const;

    template<typename T>
    uint addToParam(const char* type, const T& value);

    template<typename T>
    bool keySingleValue(const std::string& key,
                        const T& value,
                        const char *type,
                        const std::string& from);

    template<typename T>
    bool keyVector(const std::string& key,
    ...
    template<typename T>
    bool whereSingleValue(const std::string& key,
                          const T& value,
                          const char *type,
                          const std::string& oper,
                          const std::string& from);
    ...
    template<typename T>
    std::string serializeVector(const std::vector<T>& values);
};
